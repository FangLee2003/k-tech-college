...
import { ChevronLeft, ChevronRight, Heart, Home, Star, X } from 'lucide-react';
...
// Rating Component
const Rating = ({ rating = 1, maxRating = 5 }) => (
  ...
  return (
    <div className="relative w-full max-w-sm mx-auto">
      <div className="relative h-64 bg-gray-100 rounded-lg overflow-hidden">
        ...
        <button
          onClick={prevSlide}
          className="absolute left-2 top-1/2 transform -translate-y-1/2 bg-white bg-opacity-70 hover:bg-opacity-90 rounded-full p-2"
        >
          <ChevronLeft size={20} />
        </button>
        ...
        <button
          onClick={nextSlide}
          className="absolute right-2 top-1/2 transform -translate-y-1/2 bg-white bg-opacity-70 hover:bg-opacity-90 rounded-full p-2"
        >
...
const Accordion = ({ items, allowMultiple = false }) => {
  const [openItems, setOpenItems] = useState(allowMultiple ? [] : [items[0]?.id]);
        ...
        <div key={item.id} className="border-b border-gray-200">
          <button
            ...
            className={`w-full px-4 py-3 text-left text-sm font-medium ${
              openItems.includes(item.id) ? 'bg-green-500 text-white' : 'bg-gray-100 text-gray-700'
            }`}
          >
            {item.title}
          </button>
    ...
    <div className="relative w-full max-w-lg mx-auto">
      ...
      <div className="mt-4 text-center">
        <p className="text-sm text-gray-600">
          <span className="font-medium">Autoplay:</span> on/off | <span className="font-medium">Thumbnails:</span> on/off
        </p>
        <p className="text-sm text-gray-600">
...
const HomeworkPractices = () => {
  ...
  const sliderImages = [
    ...
    'https://images.unsplash.com/photo-1521747116042-5a810fda9664?w=400&h=300&fit=crop',
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white">
      ...
      {/* Slider2 Section */}
      <div className="mb-8">
        <h2 className="text-lg font-semibold mb-4">Slider2</h2>
